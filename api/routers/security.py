...
async def terminate_sessions(
    ...
    acc = await _get_account(db, account_id, current_user.id)
    updated = await sec.terminate_other_sessions(_to_dict(acc))

    # Обновляем active_sessions в БД
    ...
    return {
        ...
        "message": "Сторонние сессии завершены"
    }
...
@router.post("/accounts/{account_id}/export-session")
...
async def bulk_set_2fa(
    ...
    body: { account_ids: [...], password: "...", hint: "..." }
    """
    ...
    from tasks.bulk_tasks import celery_app
    ...
    return {
        ...
        "message": f"2FA устанавливается на {len(accounts_data)} аккаунтах"
    }
